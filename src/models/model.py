...
class Model:
    def __init__(self, model, name, paramGrid = None):
        self.__model = model
        self.__name = name
    ...
    def gridSearchFit(self, X, Y,folds):
        if self.__paramGrid is None:
            ...
            self.fit(X, Y)
            return
        grid_search = GridSearchCV(estimator=self.__model, param_grid=self.__paramGrid, cv=folds, n_jobs=-1, scoring='neg_mean_squared_error', verbose=1)
